...
<script type="text/javascript" charset="UTF-8">
	...
	$(function(){
		...
		$('#CheckStatus').combotree({
			url : '${ctx}/user/checkStatusTree',
			animate : false,
			lines : !sy.isLessThanIe8(),
			width:200,
			panelWidth:200,
		...
		datagrid = $('#datagrid').datagrid({
			url : '${ctx}/user/checkUserDatagrid',
			rownumbers:true,
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			...
			frozenColumns : [ [ {
				...
				formatter:function(value,rec,i){
					var btnHtml="";
					if((null==rec.checkStatus || ""==rec.checkStatus) || (rec.checkStatus=="N")){
						<#if fmfn.checkButton(requestURI,"icon-ok")>
							btnHtml="<a href='javascript:passCheck();' plain='true'  iconcls='icon-ok' class='easyui-linkbutton l-btn l-btn-plain'><span class='l-btn-left'><span class='l-btn-text icon-ok' style='padding-left: 20px;'>通过</span></span></a>";
						</#if>
					}
					<#if fmfn.checkButton(requestURI,"icon-search")>
						btnHtml+="<a href='javascript:edit("+i+");' plain='true'  iconcls='icon-search' class='easyui-linkbutton l-btn l-btn-plain'><span class='l-btn-left'><span class='l-btn-text icon-search' style='padding-left: 20px;'>查看</span></span></a>";
					</#if>
					return btnHtml;
				}
			} ,{
			...
			onLoadSuccess : function(node, data) {
				...
				if(organ.length>0){
				   ...
				   var arr=new Array();
					...
					//alert(arr);
					if(arr.length>0){
						organTree.combotree("setValues",arr);
					}
				}
			}
		});
	...
	function edit(index) {
		...
		userForm.find('[name=loginAcct]').attr('disabled', 'true');
		userDialog.dialog('open');
	...
	function passCheck() {
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要通过当前所选用户的注册？', function(r) {
				if (r) {
					for ( var i = 0; i < rows.length; i++) {
						ids.push(rows[i].id);
					}
					$.ajax({
						url : '${ctx}/user/passCheck',
			...
			<fieldset class="my_fieldset" >
				...
				<table>
					...
					<tr>
						<td>审核状态：</td>
						<td colspan="2">
							<input name="CheckStatus" id="CheckStatus" class="basic_input"/>
						</td>
						<!-- 
						<td>所属角色1：</td>
						<td colspan="2">
	...
	<div id="userDialog" style="display: none;overflow: hidden;background:#FFFFFF">
		<form id="userForm" method="post">
			<table class="tableForm" style="margin-left:30px;margin-top:30px;">
				...
				<tr>
					<th>生&#12288;&#12288;日：</th>
					<td><input id="birthday" name="birthday"  style="width: 196px;" /></td>
				</tr>
				<tr>
					<th>微信ID：</th>
				...
				<tr>
					<th >所属项目：</th>
					<td><select id="orgId" name="orgId.id" style="width: 200px;" required="true"></select></td>
				</tr>
				...
				<tr>
					<th>任务量</th>
					<td><input id="taskNumber" name="taskNumber"  style="width: 196px;" class="easyui-numberbox" /></td>
				</tr>
